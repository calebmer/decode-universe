...
// const rawAudioExtensionRegex = /\.decraw$/;

// async function exportRecordings(
//   directory: string,
// ): Promise<void> {
//   const allFileNames = await new Promise<Array<string>>((resolve, reject) => {
//     fs.readdir(
//       directory,
...
//     allFileNames
//       // Filter out all file names that don’t have the appropriate extension.
//       .filter(fileName => rawAudioExtensionRegex.test(fileName))
//       // Remove the extension from the file name.
//       .map(fileName => fileName.replace(rawAudioExtensionRegex, ''));

//   const x = fs.createReadStream();
// }
...
//  *
//  */
// function readFileStream(filePath: string): Observable<Buffer> {
//   return new Observable<Buffer>(observer => {
//     // Create the Node.js stream we will us to read the file.
//     const stream = fs.createReadStream(filePath);
//     // Create listeners.
//     const handleData = (data: Buffer) => observer.next(data);
//     const handleError = (error: mixed) => observer.error(error);
//     const handleEnd = () => observer.complete();
//     // Add listeners.
//     stream.addListener('data', handleData);
//     stream.addListener('error', handleError);
//     stream.addListener('end', handleEnd);
//     return () => {
//       // Remove listeners.
